...
def emit_ctx_hpp(symbols: list[Symbol]) -> str:
    parts = [_CTX_HEADER]
    for sym in symbols:
...
def emit_trampolines_cpp(symbols: list[Symbol]) -> str:
    parts = [_TRAMPOLINE_HEADER]
    # Forward-declare augment_invoke (defined in registry.cpp)
...
def main():
    ...
    if not args.json_only:
        ...
        ctx_path.write_text(emit_ctx_hpp(all_symbols))
        print(f"  → {ctx_path}")

        tramp_path = out / "augment_trampolines.cpp"
        tramp_path.write_text(emit_trampolines_cpp(all_symbols))
        print(f"  → {tramp_path}")
